...
zon_alder_split %>% 
  st_drop_geometry() %>% 
  filter(!ZONING_COD %in% other) %>% 
  mutate(sfr = case_when(ZONING_COD %in% sfr_zones ~ "single-family residential",
                         T ~ "other")) %>% 
  group_by(ALD_DIST.1, sfr) %>% 
  summarize(share_of_land = sum(area_zon)/area_ald.1) %>% #denominator is wrong here (counts whole alder district)
  distinct()
